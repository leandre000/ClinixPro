...
export default function ReceptionistForm({
  ...
  useEffect(() => {
    if (receptionist) {
      setFormData({
        ...
        isActive:
          receptionist.isActive !== undefined ? receptionist.isActive : true,
      });
    }
  }, [receptionist]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let response;
      if (isEdit) {
        response = await AdminService.updateUser(receptionist.id, userData);
      } else {
      ...
      setErrors({
        submit: err.message || "Failed to save receptionist. Please try again.",
      });
    } finally {
      setLoading(false);
    }
