...
-- This script sets up the initial database structure for ClinixPro with secure user credentials
...
DROP TABLE IF EXISTS clinixpro.password_reset_tokens CASCADE;
DROP TABLE IF EXISTS clinixpro.prescription_items CASCADE;
DROP TABLE IF EXISTS clinixpro.billing CASCADE;
DROP TABLE IF EXISTS clinixpro.prescriptions CASCADE;
DROP TABLE IF EXISTS clinixpro.appointments CASCADE;
DROP TABLE IF EXISTS clinixpro.medicines CASCADE;
DROP TABLE IF EXISTS clinixpro.patients CASCADE;
DROP TABLE IF EXISTS clinixpro.companies CASCADE;
DROP TABLE IF EXISTS clinixpro.users CASCADE;

-- Create users table
CREATE TABLE clinixpro.users (
...
CREATE TABLE clinixpro.patients (
    id BIGSERIAL PRIMARY KEY,
...
CREATE TABLE clinixpro.companies (
    id BIGSERIAL PRIMARY KEY,
...
CREATE TABLE clinixpro.medicines (
    id BIGSERIAL PRIMARY KEY,
...
CREATE TABLE clinixpro.appointments (
    id BIGSERIAL PRIMARY KEY,
...
CREATE TABLE clinixpro.prescriptions (
    id BIGSERIAL PRIMARY KEY,
...
CREATE TABLE clinixpro.prescription_items (
    id BIGSERIAL PRIMARY KEY,
...
CREATE TABLE clinixpro.billing (
    id BIGSERIAL PRIMARY KEY,
...
CREATE TABLE clinixpro.password_reset_tokens (
    id BIGSERIAL PRIMARY KEY,
    token VARCHAR(255) UNIQUE NOT NULL,
    user_id BIGINT REFERENCES clinixpro.users(id) ON DELETE CASCADE,
...
CREATE INDEX idx_password_reset_tokens_token ON clinixpro.password_reset_tokens(token);
CREATE INDEX idx_password_reset_tokens_user ON clinixpro.password_reset_tokens(user_id);
...
-- Insert secure user credentials with BCrypt hashed passwords
-- Password hashes generated using BCrypt with strength 12
INSERT INTO clinixpro.users (user_id, first_name, last_name, email, password, role, isactive) VALUES
-- Admin user: admin@clinixpro.com / admin123
('ADM-001', 'System', 'Administrator', 'admin@clinixpro.com', 
 '$2a$12$LQv3c1yqBWVHxkd0LHAkCOYz6TtxMQJqhN8/LewdBPj/RK.s5uO.G', 'ADMIN', true),

-- Doctor user: doctor@clinixpro.com / doctor123
('DOC-001', 'Dr. John', 'Smith', 'doctor@clinixpro.com', 
 '$2a$12$N9qo8uLOickgx2ZMRZoMyeIjZAgcfl7p92ldGxad68LJZdL17lhWy', 'DOCTOR', true),

-- Pharmacist user: pharmacist@clinixpro.com / pharmacist123
('PHA-001', 'Sarah', 'Johnson', 'pharmacist@clinixpro.com', 
 '$2a$12$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', 'PHARMACIST', true),

-- Receptionist user: receptionist@clinixpro.com / receptionist123
('REC-001', 'Michael', 'Brown', 'receptionist@clinixpro.com', 
 '$2a$12$8K1p/a0dL1LXMIgoEDFrwOeAQGQZQZQZQZQZQZQZQZQZQZQZQZQZQ', 'RECEPTIONIST', true);

-- Insert sample companies for testing
INSERT INTO clinixpro.companies (name, contact_person, email, phone, status) VALUES
...
('HealthCare Solutions', 'Lisa Davis', 'lisa@healthcare.com', '+1234567893', 'Active'),
('BioPharma Ltd.', 'David Wilson', 'david@biopharma.com', '+1234567894', 'Active');
...
COMMENT ON TABLE clinixpro.users IS 'User accounts with secure BCrypt password hashing';
COMMENT ON COLUMN clinixpro.users.password IS 'BCrypt hashed passwords for security';
COMMENT ON TABLE clinixpro.password_reset_tokens IS 'Secure password reset token storage';

-- Display initialization completion message
DO $$
BEGIN
    RAISE NOTICE 'ClinixPro Database initialized successfully!';
